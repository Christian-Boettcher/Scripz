...
def handle_update():
	...
	shutil.move(os.path.join(".\\data\\", 'Scripz.exe'), os.path.dirname(__file__) + "\\Scripz.exe")  # Move Scripz.exe to current directory
...
class AppHeader(ft.UserControl):
	...
	def change_theme(self, e):
		...
		self.controls[0].content.controls[2].content.selected = not self.controls[0].content.controls[2].content.selected
...
class AppFooter(ft.UserControl):
	def __init__(self, page):
		self.page = page
		super().__init__()
				...
				controls=[
					ft.Container(
						content=ft.IconButton(
							...
							on_click=lambda e: self.page.dialog.open_dialog(dialog_title="Settings", dialog_type="settings", )
						),
					),
					ft.Container(
						expand=True,
...
class CustomDialog(ft.AlertDialog):
	...
	def open_dialog(self, dialog_title=None, dialog_type=None, dialog_message=None, function_ref=None):
		...
		if dialog_type == "start_up":
			...
			new_label_input = ft.TextField(value=dialog_message.label, on_submit=lambda e: function_ref(dialog_message, new_label_input.value))
			self.close_button.text = "Cancel"
			self.close_button.on_click = lambda e: self.dismiss_dialog()
			self.confirm_button.disabled = False
			self.confirm_button.text = "Save"
				...
				ft.PopupMenuButton(
					items=[
						ft.PopupMenuItem(
							icon=ft.icons.EDIT,
							text="Rename",
							on_click=lambda event, category_object=self: self.drawer.rename_nav_option(event, category_object)
						),
						
						ft.PopupMenuItem(
							icon=ft.icons.DELETE_OUTLINE,
							text="Delete",
							on_click=lambda event, category_object=self: self.drawer.remove_nav_option(event, category_object)
						),
						
						ft.PopupMenuItem(
							icon=ft.icons.MOVE_UP,
							text="Move Up",
							on_click=lambda event, category_object=self, direction="up": self.drawer.move_nav_option(event, category_object, direction),
							disabled=True,
						),
						...
						ft.PopupMenuItem(
							...
							text="Move Down",
							on_click=lambda event, category_object=self, direction="down": self.drawer.move_nav_option(event, category_object, direction),
							disabled=True,
						),
					],
				),
...
class ScriptObject(ft.UserControl):
...
class ScriptContainer(ft.UserControl):
	def __init__(self, page):
		super().__init__()
		global GEMINI_ENABLED
		self.page = page
	...
	def confirm_delete(self, script):
		...
		for index, x in enumerate(self.scripts.controls):
			...
			try:
				...
				for i, item in enumerate(existing_data.get(self.container_title.value, [])):
					if item.get("script_name") == script.script_name and item.get("script_value") == script.script_value:
						index_to_remove = i
						break
